(() => {
  ...
  const weddingDate = new Date("2026-06-20T09:00:00+07:00").getTime();
  const sliderImages = [
  ...
  const getAudio = () => {
    if (!audio) {
      audio = new Audio("./assets/audio/minang-placeholder.wav");
      audio.loop = true;
      audio.preload = "none";
      window.audio = audio;
    }
  ...
  const setupReveals = () => {
    const addMotion = (selector, effect) => {
      $$(selector).forEach((node) => {
        node.classList.add("reveal", effect);
      });
    };

    addMotion(".script-title", "fade-soft");
    addMotion(".date-card", "zoom-out");
    addMotion(".countdown > div", "zoom-in");
    addMotion(".section-card .section-kicker", "fade-soft");
    addMotion(".section-card h2", "zoom-out");
    addMotion(".section-card > p:not(.section-kicker):not(.closing-names)", "fade-up");
    addMotion(".profile__photo--bride", "fly-left");
    addMotion(".profile__photo--groom", "fly-right");
    addMotion(".profile-separator", "zoom-in");
    addMotion(".timeline article:nth-child(odd)", "fly-left");
    addMotion(".timeline article:nth-child(even)", "fly-right");
    addMotion(".event-card:nth-of-type(odd)", "fly-left");
    addMotion(".event-card:nth-of-type(even)", "fly-right");
    addMotion(".map", "zoom-out");
    addMotion(".gallery-item:nth-child(odd)", "zoom-in");
    addMotion(".gallery-item:nth-child(even)", "zoom-out");
    addMotion(".gift-card:nth-child(odd)", "fly-left");
    addMotion(".gift-card:nth-child(even)", "fly-right");
    addMotion(".wishes article", "fade-up");

    $$(".reveal").forEach((node, index) => {
      node.style.setProperty("--reveal-delay", `${Math.min((index % 8) * 120, 720)}ms`);
