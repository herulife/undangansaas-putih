document.addEventListener("DOMContentLoaded", function() {
  ...
  const galleryImages =  document.querySelectorAll(".gallery,#nav-cerita ul img,#nav-cerita .story img");
  if(imageSlider == null) imageSlider = document.querySelector('.generateImageSlider');;
  ...
  const goToSlide = (index, withTransition = true) => {
    setBackgroundImage();
    if (withTransition) {
      imagesContainer.style.transition = `transform ${transitionTime}ms ease-in-out`;
    } else {
  ...
  const nextSlide = () => {
    ...
    if (currentIndex === totalSlides - 1) {
      ...
      setTimeout(() => isTransitioning = false, transitionTime);
    }
  };
