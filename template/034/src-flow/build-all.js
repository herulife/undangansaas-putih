...
const SOURCE_URL = 'https://share.linkundangan.com/invcode/wedding-premium034?to=Tamu+Undangan';
...
function shouldSkip(url) {
  ...
  if (url.includes('${') || url.includes('{{') || /%7B|%7D/i.test(url)) return true;
  if (skipHosts.has(parsed.hostname)) return true;
  if (/\/css\/coffee\.css(?:\?|$)/i.test(parsed.pathname + parsed.search)) return true;
  ...
  if (parsed.hostname === 'share.linkundangan.com' && /^\/invcode\//.test(parsed.pathname) && parsed.pathname !== '/invcode/wedding-premium034') return true;
...
function patchHtml(html) {
  return html
    ...
    .replace(/<link[^>]+href=(["'])(?:\.\/assets\/css\/[^"']*coffee[^"']*|\/css\/coffee\.css[^"']*)\1[^>]*>\s*/gi, '')
    .replace(/<div\s+class=(["'])coffee\1>\s*<div><\/div>\s*<div><\/div>\s*<div><\/div>\s*<\/div>\s*<div\s+class=(["'])plate\2><\/div>/gi, '')
    .replace(/<iframe[^>]+(?:youtube|instagram)[\s\S]*?<\/iframe>/gi, '<div class="local-disabled-embed">Embed eksternal dimatikan untuk mode lokal.</div>')
    .replace(/<iframe([^>]+src=["']https:\/\/maps\.google\.com\/[^"']+["'][^>]*)>/gi, (match, attrs) => {
      let next = attrs;
      if (!/\bloading=/i.test(next)) next += ' loading="lazy"';
      if (!/\breferrerpolicy=/i.test(next)) next += ' referrerpolicy="no-referrer-when-downgrade"';
      return `<iframe${next}>`;
    })
    ...
    .replace(/href=(["'])https:\/\/maps\.google\.com\/[^"']+\1/gi, (match) => match)
    .replace(/href=(["'])https:\/\/share\.linkundangan\.com\/(?:guestbook|reservation)[^"']*\1/gi, 'href="#" data-local-disabled="form"')
    .replace(/href=(["'])https:\/\/www\.linkundangan\.com\/(?:guestbook|reservation)[^"']*\1/gi, 'href="#" data-local-disabled="form"')
    .replace(/fetch\('https:\/\/share\.linkundangan\.com\/reservation\/get-count-rsv\/'\+\d+\)/g, "Promise.resolve(new Response(JSON.stringify({ numb_of_reservations: 0 }), { status: 200, headers: { 'content-type': 'application/json' } }))")
    .replace(/socket\s*=\s*new ReconnectingWebSocket\(["']wss:\/\/invity\.link:99["'][\s\S]*?\);/g, 'socket = new EventTarget(); socket.readyState = 0; socket.send = function() {};')
    .replace(/initSocket\(\);/g, '/* initSocket disabled in local mode */')
    .replace(/loadComment\(\s*!!\[\]\s*\);/g, '/* loadComment disabled in local mode */')
    ...
    .replace(/https:\/\/share\.linkundangan\.com\/invcode\/wedding-premium034/gi, './')
...
async function copyFallbackAssets() {
  const fallbackAudio = path.resolve(ROOT, '..', '..', 'apps', 'web', 'public', 'template-assets', 'wedding-premium050-original', 'assets', 'audio', 'background-music.mp3');
...
async function writeLocalRuntime() {
  ...
  const localJson = ${JSON.stringify(localJson, null, 2)};
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const loadMore = document.querySelector('#loadmorefeed');
    if (loadMore) {
